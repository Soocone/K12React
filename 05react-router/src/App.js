...
react-router-dom
  ...
  랜더링하고, 그 상태를 관리하기 위해 내부적으로 state나 props를 
  관리할 수 있게 해주는 도구가 react-roouter-dom이다
*/
import './App.css';
import {BrowserRouter, Route, Switch, NavLink, useParams} from "react-router-dom";
/*
BrowserRouter: 리액트 라우터 돔을 적용하고 싶은 컴포넌트의 최상위 컴포넌트를
  감싸주는 Wrapper(래퍼)로 사용한다.
Route
  : URL에 따른 적당한 컴포넌트를 랜더링 하기 위해 사용한다.
Switch
  : URL과 일치하는 첫번째 컴포넌트가 발견되면 나머지 컴포넌트는 아예
  랜더링 하지 않는 역할을 한다. 즉 최초로 발견되는 컴포넌트 하나만 랜더링한다.
Link
  : a 태그를 통한 링크는 클릭할때마다 화면이 새로고침되어 갱신된다.
  React는 화면의 깜빡임이 없어야하므로, 링크를 눌렀을때 reload되지 않도록 처리해준다.
*/
...
//Topics의 메뉴 링크를 구성하기 위한 JSON 배열 생성
let contents= [
  ...
  return(
    <div>
      ...
      <ul>
        ...
        {/* <Switch>
          <Route path="/Topics/free"><h2>자유게시판리스트</h2></Route>
          <Route path="/Topics/qna"><h2>QNA게시판리스트</h2></Route>
          <Route path="/Topics/faq"><h2>FAQ게시판리스트</h2></Route>
        </Switch> */}
    ...
    <BrowserRouter>
      <div className="App">
        ...
        exact
          ...
          라우팅 된다. 만약 포함하지 않으면 Topics를 눌렀을때 Home이 보이는 
          문제가 발생한다.
        */}
        ...
        <h3>Switch 적용하기</h3>
        {/* 
        Switch 컴포넌트를 사용하는 경우 첫번째 Route에 excat를 기술하지 않으면
        /가 모든 URL을 포함하는 형태이므로 아래쪽에 정의된 컴포넌트는 렌더링 되지 않는다.
