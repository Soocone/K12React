...
/*
외부 JS파일로 모듈화한 컴포넌트를 해당 문서로 import 하기 위한 구문으로
...
class App extends Component {
  ...
  render(){
    ...
    ✌ '==='는 엄격한 비교를 하는 것으로 알려져 있다 ([값 & 자료형] -> true). 타입까지 비교!!
    */
    if(this.state.mode ==='welcome'){
      ...
      */
      ...
      var i = 0;
      while(i< this.state.contents.length){
        let _readData = this.state.contents[i];
      ...
      */
      ...
      //여기서 처리하면 렌더링이 두번(밑에서 mode값을 바꿔줄때 1번, 여기서 1번) 되므로 비효율적이다.
    }
    ...
    return (
      <div className="App">
        ...
        <Buttons onChangeMode={(btn_mode)=>{
          ...
          if(btn_mode==='delete'){
            //리액트에서 confirm()을 사용할때는 반드시 window를 붙여야 한다.
