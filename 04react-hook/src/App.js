...
class ClassComponent extends React.Component{
  ...
  */
  ...
  2. getDerivedStateFromProps()라는 함수도 있는데, render가 호출되기 전에
  전달된 props를 통해 state를 변경하는 역할을 한다. 또한 반드시 반환값이 있어야 한다.
  */
  ...
  //render()가 호출된 후 자동으로 호출되는 수명주기 함수
  componentDidMount(){
    console.error("ClassComponent => componentDidMount() 호출됨");
  }
  /*
  최초 렌더링 시에는 호출되지 않고, state값이 변경에 의해 재 랜더링이 될때 호출된다.
  ...
  */
  ...
  /*
  클래스형 컴포넌트에서는 2개 이상의 state를 필요하면 컴마로 구분하여 추가하면 된다.
  */
 render(){
    ...
    <div className="container">
      ...
      */}
      ...
      {/* 
        ...
        사용해야 하지만, 화살표함수(Arrow Function)를 사용하면 별도의 바인딩이 필요 없다.
      */}
    </div>
   );
 } 
...
/*
함수형 컴포넌트
  : 출력할 내용을 즉시 return하면 된다. 함수안에 또다른 함수를 사용할 수 없으므로 
  render() 함수가 별도로 존재하지 않고, 자기 자신이 render() 역할을 한다.
*/
function FuncComponent(props){

  /*
  함수형 컴포넌트에서는 return이 render()의 역할을 하므로 특정 함수를
  선언하거나 해서 랜더링 전 전처리를 할 수 있다.
  */
  ...
  */
  ...
  //=> 반환값이 배열이므로 위 세 줄의 코드를 아래와 같이 변경할 수 있다.
  var [nowDate, setDate] = useState((new Date()).toString());

  /*
  해당 컴포넌트가 렌더링 된 후 자동으로 호출된다. 해당 함수는 첫번째 인자로
  반드시 함수가 와야한다.
  클래스형 컴포넌트는 마운팅 단계와 업데이트 단계에서 사용하는 별도의 
  수명주기 함수가 있지만, 함수형 컴포넌트에는 별도로 존재하지 않는다.
  */
  useEffect(function(){
  ...
  return (
    <div className="container">
      ...
      <p>Date: {nowDate}</p>
      {/* 
      Hook을 통해 생성된 numberState의 2번째 인자를 serNumber라는 함수명으로
